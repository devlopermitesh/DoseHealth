...
// import Carouselcustom from "@/components/customeComponets/Carousel";
const Page = () => {
  ...
  const onSubmit = async (data: z.infer<typeof loginSchema>) => {
   // request to check number api 
   //if user exits
     //send otp to email
      //on verify page i will use SignIn(next-auth) to send otp and mobilenumber
try {
  setIssubmitting(true)
  const updateNumber=data.phoneNumber.slice(2);
  const response = await axios.post("/api/check-mobile-number", { phoneNumber: updateNumber });
  if (response.data.success) {
    console.log("verification code is sended")
    alert("verification code is sended")
    toast({
      title: "Verification code is sended",
      description: "verification code is send to your email please verify in one hour",
      variant: "default",
    })
router.replace(`/veirfy/${data.phoneNumber}`);
  }
} catch (error) {
